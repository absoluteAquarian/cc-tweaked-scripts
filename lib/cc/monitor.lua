...
local function foreach_monitor(func)
    for _, monitor in native.pairs({ peripheral.find("monitor") } --[[@as table[]=]]) do
        func(monitor)
    end
end

--- Forces all connected monitors to display a blue screen with the given title and description
--- @param title string?  The title to display on the blue screen (defaults to <code>"Error"</code>)
--- @param desc string?  The description to display on the blue screen (defaults to <code>"X_x"</code>)
